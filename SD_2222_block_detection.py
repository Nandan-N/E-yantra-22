...
bridge = CvBridge()
hMin, sMin, vMin, hMax, sMax, vMax = 11 , 112 , 0 , 24 , 255 , 255
...
def findCenter(img) :
	...
	x, y = getContours(mask) # returns x and y coordinates of center of shape
	return x, y
...
def getContours(img) :
	...
	for cnt in contours : # contours - array of contours detected in image
		...
		if area > 500 : # selects only contours without too much noise (contours with area > 500 units)
			...
			cv2.rectangle(img, (x,y), (x+w, y+h), (0, 255, 0), 2) # bounding rectangle (green for each detected shape)
...
class Edrone():
				...
				for i in l:
					...
					while not (max(max(self.prev_error),-min(self.prev_error)) < 0.2 and not self.isfrstflght):
						...
						if not self.isBox:
							...
							self.setpoint = self.boxPosition
							self.setpoint[2] = 23
	...
	def imageCallback(self, ros_image):
		...
		if x != 0 and y != 0 :
			print('box detected')
			self.isBox = True
			self.boxPosition = self.drone_position

		# output
		#print(x, y)
			
		#imgStacked = stackImages(0.5, ([img, imgResult]))
...
if __name__ == '__main__':
	...
	time.sleep(2)
	r = rospy.Rate(1) #specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz

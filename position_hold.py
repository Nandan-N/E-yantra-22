...
'''
Team ID: 2222
Author List: Pratham Patil, Nandan N, Sriram Radhakrishnan, Samuel Thomas
...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint = [2, 2, 20, 0.0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
	...
	def pitch_set_pid(self, pitch):
		self.Kp[1] = pitch.Kp * 0.001 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[1] = pitch.Ki * 0.008
		self.Kd[1] = pitch.Kd * 0.5

	def roll_set_pid(self,roll):
		self.Kp[0] = roll.Kp * 0.001 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[0] = roll.Ki * 0.008
		self.Kd[0] = roll.Kd * 0.5


	def yaw_set_pid(self, yaw):
		self.Kp[3] = yaw.Kp * 0.001 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[3] = yaw.Ki * 0.008
		self.Kd[3] = yaw.Kd * 0.5
	...
	def run(self):
			while self.setpoint:
				...
				if len(self.setpoint) == 4:
					print("TARGET LOCKED")
				while not ((max(max(self.prev_error),-min(self.prev_error)) < 0.5 ) and not self.isfrstflght):
